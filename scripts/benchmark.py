...
print(f"Benchmarking results in: {results_dir}, output to {out_dir}")
methods = ["attention"]
...
for eval_sequence in EVAL_DATASETS:
    ...
    track_data_gt = read_txt_results(
        str(results_dir/ f"{sequence_name}" / "gt" / f"{sequence_name}.gt.txt")
    )
    ...
    for method in methods:
        ...
        track_data_pred = read_txt_results(
            str(results_dir / f"{sequence_name}" / f"{method}" / f"{sequence_name}.txt")
        )
