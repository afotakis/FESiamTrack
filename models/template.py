...
class Template(LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.pose_mode is False:
            ...
            # )
            # --- NEW: log a figure histogram instead (avoids TB’s histogram path) ---
            fig = plt.figure()
            ax = fig.add_subplot()
            ax.hist(np.asarray(self.epe_l2_hist, dtype=np.float32), bins=50)
            ax.set_xlabel("EPE (px)")
            ax.set_ylabel("Count")
            self.logger.experiment.add_figure("EPE_hist/val", fig, self.global_step)
            plt.close(fig)
