...
class RAFTDisplacementSequenceLoss(nn.Module):
    """
    RAFT-style sequence loss for per-step displacement predictions.

    Each unroll step is exponentially weighted, with later predictions
    receiving higher weight. Unlike the original truncated loss used in
    this project, no validity mask is applied.
    """
    ...
    def forward(self, displacement_preds, displacement_gt):
        """
        displacement_preds: list[(B, 2)]
        displacement_gt:    list[(B, 2)]
        returns:
            scalar loss
        """
        n_predictions = len(displacement_preds)
        if n_predictions == 0:
            raise ValueError("RAFTDisplacementSequenceLoss requires at least one prediction")
        if n_predictions != len(displacement_gt):
            raise ValueError("Prediction and target sequence lengths must match")
        
        loss = 0.0
        for i, (pred, gt) in enumerate(zip(displacement_preds, displacement_gt)):
            weight = self.gamma ** (n_predictions - i - 1)
            loss = loss + weight * (pred - gt).abs().mean()
...
class L1TruncatedWithLogGrad(nn.Module):
    """
    L = mask * ( |ex|+|ey| + lambda * | g^T e | )
    where g = [gx, gy] is ∇x log(I) evaluated per sample (e.g., at patch center).
    """
    def __init__(self, patch_size=31, grad_weight=1.0):
        super().__init__()
        self.patch_size = patch_size
        self.grad_weight = float(grad_weight)

    def forward(self, y, y_hat, grad_logI_xy):
        """
        y, y_hat: (B,2)
        grad_logI_xy: (B,2)  -> [gx, gy] for each sample
        returns:
            loss: (B,)
            mask: (B,)
        """
        # same validity mask as your L1Truncated
        mask = ((torch.abs(y) <= self.patch_size / 2.0).all(dim=1)).float().detach()
        mask = mask.requires_grad_(True)

        e = y - y_hat                       # (B,2)
        l1 = torch.abs(e).sum(dim=1)        # (B,)

        # dot = gx*ex + gy*ey
        dot = (grad_logI_xy * e).sum(dim=1) # (B,)
        loggrad_term = torch.abs(dot)       # (B,)  (L1-style)

        loss = (l1 + self.grad_weight * loggrad_term) * mask
        return loss, mask
