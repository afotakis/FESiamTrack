...
# Photometric augmentation for grayscale reference patches.
# Brightness is expressed in 8-bit pixel units; if the patch is normalized
# to [0, 1], it is internally converted to +/- 10 / 255.
PHOTOMETRIC_BRIGHTNESS_DELTA = 10.0
PHOTOMETRIC_CONTRAST_MIN = 0.8
PHOTOMETRIC_CONTRAST_MAX = 1.2
...
def parse_bins_from_representation(rep: str) -> int:
    """
    Extract the number of temporal bins from a representation string.

    Examples:
        'time_surfaces_v2_5'  -> 5
        'time_surfaces_v2_10' -> 10
        'voxel_grids_5'       -> 5
    """
    token = rep.split("_")[-1]
    try:
        ...
        raise ValueError(f"Cannot parse num bins from representation '{rep}'")
...
class TrackData:
    ...
    def __init__(self, track_tuple, config):
        ...
        self.last_aug_brightness, self.last_aug_contrast = 0.0, 1.0
        ...
        # print(f"Loaded track with {self.track_data.shape[0]} entries.")
        # print(f"Track data {self.track_data[:]}")
        ...
        if "grayscale" in config.representation:
            ...
            bins = parse_bins_from_representation(config.representation)
            self.channels_in_per_patch = bins
            # in v2, we have separate temporal bins for each event polarity
            if "v2" in config.representation:
                self.channels_in_per_patch *= 2  

    def augment_reference_photometric(self, x_ref):
        """Apply per-unroll brightness/contrast augmentation to the frame patch only."""
        contrast = float(torch.empty(1).uniform_(
            PHOTOMETRIC_CONTRAST_MIN,
            PHOTOMETRIC_CONTRAST_MAX,
        ).item())

        # read_input/get_patch_voxel may return either [0, 1] or [0, 255].
        # Keep the additive brightness in the correct scale.
        normalized = bool(x_ref.detach().max() <= 1.5)
        brightness = float(torch.empty(1).uniform_(
            -PHOTOMETRIC_BRIGHTNESS_DELTA,
            PHOTOMETRIC_BRIGHTNESS_DELTA,
        ).item())
        if normalized:
            ...
            clamp_min, clamp_max = 0.0, 255.0

        # Contrast around the patch mean avoids introducing an unnecessary
        # global intensity shift from the contrast operation itself.
        mean = x_ref.mean(dim=(-2, -1), keepdim=True)
        x_aug = (x_ref - mean) * contrast + mean + brightness
        x_aug = x_aug.clamp(clamp_min, clamp_max)

        self.last_aug_brightness = brightness
        self.last_aug_contrast = contrast
        return x_aug
    ...
    def get_next(self):
        ...
        # print(f"Track idx {self.track_idx}, time idx {self.time_idx}, u_center {self.u_center}, u_center_gt {self.u_center_gt}")
        ...
        # Photometric augmentation: apply only to the grayscale reference patch,
        # not to the event representation. This is sampled every get_next() call,
        # therefore every unroll step receives a new brightness/contrast sample.
        x_ref = self.x_ref
        if self.config.augment:
            x_ref = self.augment_reference_photometric(self.x_ref.clone())

        x = torch.cat([x, x_ref], dim=0)
        ...
        # print(f"Track {self.track_idx}, time {self.time_idx}, x shape {x.shape}, y {y}")
...
def _parse_num_stem(path: str) -> int:
...
class MFDataModule(LightningDataModule):
    def __init__(
        ...
        **kwargs,
        ...
        for split_name in ["train", "test"]:
            ...
            #print(f"[MFDataModule] Found {len(track_tuples)} tuples in {split_name}")
            # Shuffle and trim
            ...
            track_tuples_array[:, 1] = track_tuples_array[:, 1].astype(np.int64)
            track_tuples = []
    ...
    def get_frame_paths(track_path):
        images_dir = Path(
            ...
            .replace("_reloaded", "").replace("_extra", "")
    ...
    def get_event_paths(track_path, rep, dt):
        ...
        return [
            ...
            if 400000 <= _parse_num_stem(event_p) <= 900000
        ]
    ...
    def setup(self, stage=None):
        ...
        #one_track = [self.split_track_tuples["train"][0]]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.dir_representation.exists():
            print(
                f"{self.representation} has not yet been generated for a dt of {self.dt}, EDS"
            )
            exit()
...
class ECSubseq(SequenceDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.dir_representation.exists():
            print(
                f"{self.representation} has not yet been generated for a dt of {self.dt}, EC"
            )
            exit()
...
class SequenceDatasetV2(ABC):
    ...
    def initialize_time_and_input_paths(self, pose_mode, dt_or_r):
        if pose_mode:
            ...
            #print(self.event_representation_paths)
        else:
    ...
    def get_even_event_paths(sequence_dir, representation, r):
        ...
        :return: sorted list of event paths for a given representation and time-delta
        """
        #print(str(sequence_dir / "events" / f"pose_{r:.0f}" / f"{representation}" / "*.h5"))
        return sorted(glob(str(sequence_dir / "events" / f"pose_{r:.0f}" / f"{representation}" / "*.h5")))
