...
def augment_translation(x, y, max_translation=3, translation_xy=None):
    """
    Translate a patch (affine warp) and adjust displacement label accordingly.

    We use torchvision's affine translation (positive tx shifts content right,
    positive ty shifts content down). This effectively changes the target offset
    in the translated patch as:
        y_aug = y + [tx, ty]
    when translation is applied AFTER (scale, rotation).

    Args:
        x: (C, P, P) tensor patch
        y: (2,) tensor displacement (dx, dy)
        max_translation: max absolute translation in pixels
        translation_xy: optional (tx, ty) to apply directly

    Returns:
        x_aug, y_aug, t   where t is a torch tensor shape (2,)
    """
    if translation_xy is None:
        tx = random.randint(-max_translation, max_translation)
        ty = random.randint(-max_translation, max_translation)
    else:
        tx, ty = translation_xy

    # affine translate in pixels
    x_aug = affine(
        x,
        angle=0.0,
        translate=[tx, ty],
    ...
    t = torch.tensor([tx, ty], dtype=y.dtype, device=y.device)
    y_aug = y + t
    return x_aug, y_aug, t


def unaugment_translation(y: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
    # make sure t is on same device
    return y - t.to(device=y.device, dtype=y.dtype)